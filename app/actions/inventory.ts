...
export async function syncAvailability() {
  ...
  try {
    ...
    if (roomTypes.length === 0) return { success: false, message: "No room types found to update." };
    ...
    return { success: false, message: error.message || "Failed to sync availability" };
  }
}
